...
export interface GlobalManagerData {
...
const checkGlobalInstallation = async (manager: PackageManager): Promise<boolean> => {
  ...
  if (cached !== undefined && 'isGlobalInstalled' in cached) {
    return cached.isGlobalInstalled;
  }
 ...
 *
 * @returns {Promise<GlobalManagerData[] | null>} A promise that resolves to an array of global manager data
 * including the name and version of each installed package manager, or `null` if none are installed.
 */
export const defineGlobalManagers = async (): Promise<GlobalManagerData[] | null> => {
  const cacheKey = 'globalManagers';
  const cached = cache.get(cacheKey);
  if (cached !== undefined && 'globalManagers' in cached) {
    return cached.globalManagers;
  }

  const managers = Object.keys(lockFiles) as PackageManager[];

  const globalManagers = await Promise.all(
    managers.map(async (manager): Promise<GlobalManagerData | null> => {
  ...
  const result = cleanNulls<GlobalManagerData>(globalManagers);
